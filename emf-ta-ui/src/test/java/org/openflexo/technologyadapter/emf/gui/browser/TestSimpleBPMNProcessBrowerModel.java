...
public class TestSimpleBPMNProcessBrowerModel extends OpenflexoTestCaseWithGUI {
	protected static final Logger logger = Logger.getLogger(TestSimpleBPMNProcessBrowerModel.class.getPackage().getName());

	static EMFTechnologyAdapter technologicalAdapter;
	static EMFModelResource simpleBPMNProcessResource = null;
	static EMFModel simpleBPMNProcess = null;

	private static String BPMN_META_MODEL_URI = "http://www.omg.org/spec/BPMN/20100524/MODEL-XMI";

	private static SwingGraphicalContextDelegate gcDelegate;

	static String simpleBPMNProcessURI = "http://openflexo.org/emf-test/TestResourceCenter/EMF/Model/bpmn/BasicExample.bpmn";
	...
	public void loadBPMNMetaModel() {
		...
		for (FlexoResourceCenter<?> resourceCenter : serviceManager.getResourceCenterService().getResourceCenters()) {
			...
			EMFMetaModelResource metaModelResource = metaModelRepository.getResource(BPMN_META_MODEL_URI);
	...
	@TestOrder(2)
	@Category(UITest.class)
	public void loadBasicExampleProcess() {
		...
		for (FlexoResourceCenter<?> resourceCenter : serviceManager.getResourceCenterService().getResourceCenters()) {
			...
			System.out.println("Loading from RC :" + simpleBPMNProcessURI);

			EMFModelResource modelResource = modelRepository.getResource(simpleBPMNProcessURI);

			if (modelResource != null) {
				simpleBPMNProcessResource = modelResource;
				System.out.println("Found resource ");
			}
			else {
				System.out.println("Not found: " + simpleBPMNProcessURI);
		...
		assertNotNull(simpleBPMNProcessResource);

		simpleBPMNProcess = simpleBPMNProcessResource.getModel();
		assertNotNull(simpleBPMNProcess);
		assertNotNull(simpleBPMNProcess.getMetaModel());

		for (IFlexoOntologyClass<EMFTechnologyAdapter> c : simpleBPMNProcess.getClasses()) {
			System.out.println(" >>>> " + c.getName());
		}

		for (IFlexoOntologyIndividual<EMFTechnologyAdapter> c : simpleBPMNProcess.getIndividuals()) {
			System.out.println(" > " + c.getName());
		}
	...
	public void TestCreateOntologyBrowser() {
		...
		OntologyBrowserModel<EMFTechnologyAdapter> obm = new EMFModelBrowserModel(simpleBPMNProcess);
	...
	public void TestCreateEMFModelView() {
		...
		EMFModelView modelView = new EMFModelView(simpleBPMNProcess, null, null);
		currentDate = System.currentTimeMillis();
	...
	public static void initGUI() {
		gcDelegate = new SwingGraphicalContextDelegate(TestSimpleBPMNProcessBrowerModel.class.getSimpleName());
	}
